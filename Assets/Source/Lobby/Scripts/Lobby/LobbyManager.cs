...
namespace Prototype.NetworkLobby
{
    public class LobbyManager : NetworkLobbyManager 
    {
        ...
        void Start()
        {
            ...
            MenuController.Instance.PrepareLoading(playScene);
        ...
        public void ReadyToBeginMessageHandler(NetworkMessage netMsg)
        {
            ...
            var isServer = client.connection.playerControllers[0].unetView.isServer;
            GameController.Instance.StartPlaying(isServer);
        }

        ////////////////////////////////////////////////////////////////////////////////////////
        class ChatMsgSendMsg : MessageBase
        {
            public ChatMessage Message;
        }

        public void SendChatMsg(ChatMessage msg)
        {
            var message = new ChatMsgSendMsg();
            message.Message = msg;
            
            client.Send(MsgChatMsgSend, message);
        }

        public void SendChatMsgMessageHandler(NetworkMessage netMsg)
        {
            var msg = netMsg.ReadMessage<ChatMsgSendMsg>();
            if (msg != null)
            {
                var isValidMsg = ChatController.Instance.ValidateChatMessage(msg.Message);

                if (isValidMsg)
                {
                    BroadcastChatMsgToAll(msg.Message);
                }
            }
        }

        public void BroadcastChatMsgToAll(ChatMessage msg)
        {
            var message = new ChatMsgSendMsg();
            message.Message = msg;

            NetworkServer.SendToAll(MsgChatMsgSend, message);
        }

        public void ReceiveChatMsgMessageHandler(NetworkMessage netMsg)
        {
            var msg = netMsg.ReadMessage<ChatMsgSendMsg>();
            if (msg != null)
            {
                ChatController.Instance.OnMessageReceived(msg.Message);
            }
        }
        ...
        public void GamePhaseChangedMessageHandler(NetworkMessage netMsg)
        {
            var msg = netMsg.ReadMessage<GamePhaseChangedMsg>();
            if (msg != null)
            {
                GameController.Instance.ChangeState(msg.m_phase, msg.m_duration);
            }
        }
        ...
        public override void OnClientConnect(NetworkConnection conn)
        {
            ...
            conn.RegisterHandler(MsgChatMsgSend, ReceiveChatMsgMessageHandler);
